...
async function checkLanguageTool(text) {
  try {
    ...
    for (const match2 of sortedMatches) {
      if (match2.replacements && match2.replacements.length > 0) {
        const replacement = match2.replacements[0].value;
        const start = match2.offset;
        const end = match2.offset + match2.length;
...
__name(checkLanguageTool, "checkLanguageTool");
...
__name(cleanSpokenEnglish, "cleanSpokenEnglish");
...
async function onRequest2(context) {
  ...
  try {
    ...
    console.log("\u{1F4E1} AssemblyAI response status:", tokenResponse.status);
    if (!tokenResponse.ok) {
...
// ../.wrangler/tmp/pages-j6ZsIa/functionsRoutes-0.19104469260653822.mjs
var routes = [
  {
