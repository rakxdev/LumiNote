...
## Phase A: Foundation
- [x] Task 0: Local Node runtime available; baseline `npm test` green.
- [x] Task 1: PCM batch duplication fixed with test (appendPcmChunk in audio-processor.js).
...
- [x] Task 2: SyncRoom Durable Object worker + protocol tests + dry-run deploy check.
...
- [x] Task 3: /api/link/ws Pages Function proxy + DO binding in wrangler.toml.
  - Acceptance: validation tests pass; dry-run OK.
  - Files: functions/api/link/ws.js, wrangler.toml

## Checkpoint: Backend
- [x] Suite green; both deploy dry-runs pass.

## Phase C: Client
- [x] Task 4: Pairing UI (LINK button, modal, QR, join, status, reconnect) + vendored QR lib.
...
- [x] Task 5: Live relay + Remote Clipboard tray (turn/interim/clipboard handling, auto-copy).
  - Acceptance: node --check passes; suite green.

## Checkpoint: Client
- [x] Suite green; all client JS parses.

## Phase D: Proof & Ship
- [x] Task 6: Local end-to-end two-client fan-out verification.
- [x] Task 7: ADR-003 + README/DESIGN docs.
- [x] Task 8: Deploy worker + Pages; live verification; final commit.
